...
def create_argparser():
    ...
    parser.add_argument(
        "-f", "--source", help="A raw text file or a directory that contains text files"
    )
    parser.add_argument("-s", "--vocabulary_size", help="Vocabulary size")
    parser.add_argument("-w", "--skip_window", help="Skip window Size")
    return parser
...
if __name__ == "__main__":
    ...
    cmdline_args = {
        k: v
        for k, v in list(
            vars(arg_parser.parse_args()).items()
        )  # using vars() to turn object attribute to dict
        if v is not None
    }
    ...
    print("start to scan tokens from files")
