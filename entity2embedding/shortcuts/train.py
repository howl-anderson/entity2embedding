...
def create_argparser():
    ...
    parser.add_argument('-m', '--max_steps', default=-1,
                        help="Number of batches to run.")
    parser.add_argument('-d', '--log_device_placement', default=False,
                        help="Whether to log device placement.")
    parser.add_argument('-f', '--log_frequency', default=10,
                        help="How often to log results to the console.")
    parser.add_argument('-b', "--batch_size", default=128,
                        help="How much one batch holds.")
    parser.add_argument("-e", "--embedding_size", default=128,
                        help="How much embedding vector size.")
    parser.add_argument("-n", "--num_sampled", default=64,
                        help="How much negative samples are used.")
    return parser
...
def train(corpora_object, log_dir, FLAGS):
    with tf.Graph().as_default():
        with tf.device('/cpu:0'):
            ...
            train_op = word2vec.train(training_input, labels, log_dir,
...
if __name__ == "__main__":
    ...
    corpora_object = TFRecordBasedCorporaLoader.TFRecordBasedCorporaLoader(
        input_files, project['ONE_HOT_METADATA_FILE'],
